...
class flat_navigation extends navigation_node_collection {
    ...
    public function initialise() {
        ...
        //Start - Add a Certificate Button by Guang Jin
        if(is_siteadmin()){
            $url = new moodle_url($CFG->wwwroot . '/certification/index.php');
            $certification = navigation_node::create('Certification', $url);
            $flat = new flat_navigation_node($certification, 0);
            $flat->set_showdivider(true, get_string('certification'));
            $flat->key = 'certification';
            $flat->icon = new pix_icon('i/portfolio', '');
            $this->add($flat);
        }
        //End
